...
4. **PDF レポート生成**
   - `POST /api/report`
   - PDFShift でチャート画像/サマリー/サブページ抜粋を含むレポート
5. **ダッシュボード UI**
   - フィルタ (Segment / Issue / Year)
   - バブルチャート (Chart.js)
   - 主要プレイヤーランキング & 集計メトリクス
   - 詳細パネル (Notion サブページ Markdown 表示、PDF生成ボタン)
...
3. バブルチャートから任意の市場をクリックすると詳細パネルが更新
4. 詳細パネル右上「PDF レポート」で PDFShift によるレポートをダウンロード
5. Notion 側で更新があった場合は「Notion同期」ボタン or Cron による自動同期
